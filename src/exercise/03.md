...
Vocabulary:
JSX is a super set of Javascript.
Babel is a compiler.

Babel and JSX in context:
When writing react components with JSX we need a compiler to "translate" JSX into javascript. The reason is because the browser doesn't "understand" JSX therefore the element needs to be converted by a compiler like babel into a language that the browser can run.

To sum up:
in order to create and run a react element with JSX in the browser we need
*react to create the element
*reactDOM to take those react elements and create DOM nodes to display them on the browser
*babel to compile our react/JSX code into javascript that the browser can run
