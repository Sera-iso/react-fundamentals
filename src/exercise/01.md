...
Create a root element either in the HTML or JS.
To make sure its content is not just saved in memory but is displayed by the browser
we need to pick up the root element by id and then append an element (e.g. div)
with it's content to the page.
