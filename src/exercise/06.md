...
Submitting form events can be done in 2 ways:

1.targeting the `id` or the `name` of an element.
2.use React.useRef() to link a `ref` from the input to the event.
3.targeting the form via index (not very
scalable if the component has multiple elements).

It is possible to use the hook React.useState() to maintain a state over time. In this case we want to trigger an error if the user types upper case letters. The hook sets an initial error state as null and it will update it depending on conditions/events happening in our input.
